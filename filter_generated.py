...
def get_parser():
    ...
    main_parser.add_argument('--store', type=str, nargs='+',
                             default=['valid', 'connectivity'],
                             choices=['all', 'valid', 'new', 'connectivity',
                                      'fingerprint', 'process'],
...
def collect_fingerprints_and_cans(mols):
    ...
    return {'fingerprint_bits': fps_bits, 'canonical_smiles': cans,
...
if __name__ == '__main__':
    ...
    n_valid = 0
    n_non_unique = 0
    stat_heads = ['n_atoms', 'id', 'original_id', 'valid', 'duplicating',
                  'n_duplicates', 'known', 'equals', 'C', 'N', 'O', 'F', 'H', 'H1C',
                  'H1N', 'H1O', 'C1C', 'C2C', 'C3C', 'C1N', 'C2N', 'C3N', 'C1O',
    ...
    for n_atoms in res:
        ...
        if 'fingerprint' in args.store:
            if args.threads <= 0:
                ...
                results = {'fingerprint_bits': [],
                           'canonical_smiles': [],
                           'mirrored_canonical_smiles': []}
                run_threaded(collect_fingerprints_and_cans,
                             {'mols': mols},
                             {},
                             results,
                             n_threads=args.threads)
    ...
    if 'all' not in args.store:
        ...
        for key in res:
            ...
            if 'fingerprint' in args.store:
                additional_information += ['fingerprint_bits', 'canonical_smiles',
                                           'mirrored_canonical_smiles']
            if 'process' in args.store:
                additional_information += ['focus', 'type_chosen', 'type_prediction',
                                           'dist_predictions']
    ...
    with connect(target_db) as conn:
        ...
        for n_atoms in res:
            ...
            for i in range(len(d[Properties.R])):
                ...
                if 'fingerprint' in args.store:
                    data.update({'fingerprint_bits':
                                     np.array(list(d['fingerprint_bits'][i])),
                                 'canonical_smiles':
                                     str(d['canonical_smiles'][i]),
                                 'mirrored_canonical_smiles':
                                     str(d['mirrored_canonical_smiles'][i])
                                 })
                if 'process' in args.store:
                    data.update({info: d[info][i] for
                                 info in ['focus', 'type_chosen',
                                          'type_prediction', 'dist_predictions']})
